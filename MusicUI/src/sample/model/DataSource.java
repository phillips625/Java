...
public class DataSource {
    ...
    // The prepared statement will insert the COLUMN_ALBUM_ARTIST.
    ...
    // This variable will hold the one instance that ONLY this class can create
    private static DataSource instance = new DataSource();

    // Since this singleton is private, only this class will be able to create an instance of the DataSource class.
    private DataSource(){

    }

    // An object will use this method if they want to access the singleton
    public static DataSource getInstance(){
        // This code is not thread safe. A thread can run and create the first instance. Another thread could run,
        // check for null and create the second instance. The result is that we now have two instances - which is not
        // ideal. Instead, to make this thread safe, instantiate the DataSource in the declaration.
//        if (instance == null){
...
//            results = statement.executeQuery("SELECT * FROM " + TABLE_ARTISTS);
            ...
            while (results.next()){
                ...
                // Adding a bit of delay so we can see the progress bar in prooress.
                // For every retrieved artists, the code will sleep for 20ms.
                try {
                    Thread.sleep(20);
                }
...
//                artist.setName(results.getString(COLUMN_ARTIST_NAME));
            ...
            return null;
                ...
                // id - This is the id passed by SQLite
    // Getting a list of albums based on the artist id.
    public List<Album> queryAlbumsForArtistId(int id){
        try {
            // 1 - 1 corresponds to the first column in the database.
            // Setting the artist id in the prepared statement.
            ...
            while (results.next()){
                ...
                // Retrieving the artist directly.
...
//            System.out.format("Count = %d, Min = %d\n", count, min);
    ...
    public boolean updateArtistName(int id, String newName) {
        try {
            ...
            //   public static final String UPDATE_ARTIST_NAME = "UPDATE " + TABLE_ARTISTS + " SET " +
            // COLUMN_ARTIST_NAME + " = ? WHERE " + COLUMN_ARTIST_ID + " = ?";
            // 1 corresponds to the first question mark
            // 2 corresponds to the second question mark
            updateArtistName.setString(1, newName);
            updateArtistName.setInt(2, id);
            int affectedRecords = updateArtistName.executeUpdate();

            // Return true if we only updated 1 record, otherwise false.
